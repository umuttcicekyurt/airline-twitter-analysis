...
keep_fields = ["created_at", "id_str", "text","in_reply_to_status_id_str", "in_reply_to_user_id_str", "user", "quoted_status_id_str", "quote_count", "reply_count", "lang", "favorite_count", "retweeted_count", "possibly_sensitive", "is_retweet"]
...
def is_bot(user: Dict, tweet_created_at: str) -> bool:
    ...
    if followers > 0 and friends > 0 and friends / followers < 0.01:
        return True

    if followers == 0 or friends == 0:
        return True

    if user_created_at and tweet_created_at:
        try:
            user_creation_date = datetime.strptime(user_created_at, "%a %b %d %H:%M:%S %z %Y")
            tweet_date = datetime.strptime(tweet_created_at, "%a %b %d %H:%M:%S %z %Y")
            account_age_days = (tweet_date - user_creation_date).days
            ...
            for i, line in enumerate(f):
                ...
                try:
                    ...
                    if valid_tweet(tweet):
                        ...
                        if tweet["id_str"] not in seen_ids and not is_bot(user, tweet_created_at):
