...
def is_bot(user: Dict) -> bool:
    ...
    # Example bot criteria: very high posting frequency or low follower-to-friend ratio
    if followers > 0 and friends > 0 and followers / friends < 0.1:
        return True
    if statuses > 100000:  # Arbitrary threshold for high activity
        return True
    return False
...
def process_file(input_path: str, output_path: str):
    ...
    cleaned = []
    seen_ids = set()  # Track unique tweet IDs
    try:
            ...
            for i, line in enumerate(f):
                ...
                try:
                    ...
                    if valid_tweet(tweet):
                        user = tweet.get("user", {})
                        if tweet["id_str"] not in seen_ids and not is_bot(user):
